...
def prepare_dataset(csv_path: Path) -> pd.DataFrame:
    df = pd.DataFrame(pd.read_csv(csv_path)[[TEXT_COL, LABEL_COL]].dropna().copy())
    ...
    deduped = (
        df.sort_values(by=[LABEL_COL, "group_key"])
        .drop_duplicates(subset=["group_key", LABEL_COL])
        .reset_index(drop=True)
    )

    counts = deduped[LABEL_COL].value_counts()
    valid_classes = [val for val, count in counts.items() if count >= MIN_SAMPLES_PER_CLASS]
...
def train_and_evaluate(df: pd.DataFrame):
    ...
    print("Training TF-IDF + MLPClassifier pipeline...")
    clf = build_pipeline()
...
def make_serializable(o):
    if isinstance(o, dict):
        return {str(k): make_serializable(v) for k, v in o.items()}
...
def save_artifacts(model, summary, model_path: Path, report_path: Path):
    ...
    metadata = {
        ...
        "model_type": "TF-IDF_MLPClassifier",
        "architecture": "TF-IDF → MLPClassifier(128,64)",
    ...
    report_path.write_text(json.dumps(serializable_summary, indent=2))
